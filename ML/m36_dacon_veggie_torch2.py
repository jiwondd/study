...
all_input_list = sorted(
    glob.glob('./_data/dacon_veggie/train_input/*.csv'))
all_target_list = sorted(
    glob.glob('./_data/dacon_veggie/train_target/*.csv'))
...
test_input_list = sorted(
    glob.glob('./_data/dacon_veggie/test_input/*.csv'))
test_target_list = sorted(
    glob.glob('./_data/dacon_veggie/test_target/*.csv'))
...
with zipfile.ZipFile("torch_sub_drop2.zip", 'w') as my_zip:
    for i in filelist:
        my_zip.write(i)
    my_zip.close()